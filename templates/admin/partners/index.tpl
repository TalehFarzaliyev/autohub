<div class="panel panel-white">
	<div class="panel-heading">
		...
		<div class="heading-elements">
			<div class="btn-group">
				...
				<a href="{site_url_multi('admin/tv')}?lang_id={$language.id}" class="{$language.class}"><img src="{base_url('templates/admin/assets/images/flags/')}{$language.code}.png" alt="{$language.name}"> {$language.name} <span class="label bg-slate-700">{$language.count}</span></a>
				{/foreach}
				{/if}
			</div>
	...
	<div class="table-toolbar-area" style="display: none; border-bottom: 1px solid #dfdfdf; background: #f5f5f5; padding: 10px;">
		<div class="row">
			<div class="col-md-11" style="padding-top: 5px">
				...
				<label class="checkbox-inline"><input type="checkbox" class="styled table-column-checkbox" checked="checked" value="{$column}">{$text.partners["partners_table_head_{$column}"]}</label></a>
				{/foreach}
			</div>
			<div class="col-md-1">
	...
	{/if}
	{form_open_multipart('http://192.168.0.123/cms/admin/partners/delete', 'class="form-horizontal" id="form-list"')}
	{$table}
	{form_close()}
