...
class City extends Admin_Controller {
	...
	public function create()
	{
		...
		$this->data['country'] = $this->Country_model->get_rows($this->data['columns'], $this->data['filter'], [], []);

		$country = [];

	    if($this->data['country']){
	    	$country[0] = $this->data['text'][$this->controller][$this->controller.'_form_label_please_select'];
	      foreach ($this->data['country'] as $row) {
	        $country[$row['id']] = $row['name'];
	      }

	    }
		//echo "<pre>"; print_r($this->data['parent']); die();
				...
				$this->data['form_field']['general'] = [
					'country_id' => [
                		...
                		'id'		=> 'country_id',
						  ...
						  'data-width'	=> '100%',
						  'options'		=> $country,
				         'selected'      => (set_value('country_id')) ? set_value('country_id') : $this->data['general']->country_id,
				         'validation'	=> [
			                'rules' => 'required',
			                'errors' => [
	...
	public function ajax_get_parent_to_city($id)
	{
		$response = [];
		if($id and ctype_digit($id))
		{
			$this->data['columns'] 		= ['*'];
			$this->data['filter']['lang_id'] 	= ($this->input->get('lang_id') != NULL) ? (int)$this->input->get('lang_id') : $this->data['current_lang_id'];
			$this->data['filter']['country_id'] 	= $id;
			$city_ids = $this->{$this->model}->get_rows($this->data['columns'], $this->data['filter'], [], []);
			...
			if($city_ids){
				...
				$ids = implode(',', $ids);
				$categories = $this->{$this->model}->get_rows('id,name',['city_id IN('.$ids.')'=>null,'lang_id'=>$this->data['current_lang_id'], 'status'=>'1']);
