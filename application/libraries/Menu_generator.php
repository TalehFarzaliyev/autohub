...
class Menu_generator {
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) )
	        {
		        ...
		        if ($has_children)
		        {
		        	if ( is_null($item[$this->menu_parent]) && $this->parentl1_tag_open != '' )
		        	{
						...
						$item_anchor = $this->parent_anchor != '' ? $this->parent_anchor : $this->parent_anchor;
		        	}
		        	else
		        	{
	...
	{
		$segment = site_url(implode('/', $this->ci->uri->segment_array()));
		
		if ( ($this->item_active != '' && $slug == $this->item_active && empty($segment)) || site_url($slug) == $segment)
		{
			$doc = new DOMDocument();
			$doc->loadHTML($html);
