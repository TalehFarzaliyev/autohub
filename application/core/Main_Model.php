...
class Main_Model extends CI_Model
{
    ...
    private function _has_one($request)
    {
        ...
        $this->_database->join($this->db->dbprefix($relation['foreign_table']), $relation['foreign_table'].'.'.$relation['foreign_key'].' = '.$this->table.'.'.$relation['local_key'], 'left');
        return TRUE;
    }
    ...
    public function remove($id)
	{
		...
		foreach ($this->relation_tables as $relation_table)
		{
			if($relation_table['delete'] === TRUE)
			{
				$this->db->delete($relation_table['name'], [$relation_table['column'] => $id]);
			}
		}
    }
    ...
    public function remove_all()
	{
		...
		if($query->num_rows() > 0)
		{
			...
			{
				$this->db->delete($this->table, [$this->primary_key => $row->id]);
				if(isset($this->relation_tables) && !empty($this->relation_tables))
				{
					foreach ($this->relation_tables as $relation_table)
					{
						if($relation_table['delete'] === TRUE)
						{
							$this->db->delete($relation_table['name'], [$relation_table['column'] => $row->id]);
						}
					}
				}
			}
	...
	{
		$this->db->delete($this->relation_tables[0]['name'], [$this->relation_tables[0]['column'] => $id]);
	}
}